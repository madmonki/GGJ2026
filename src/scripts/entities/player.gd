...
@onready var mask_scene = preload("res://src/scenes/props/mask.tscn")
...
var doom_duration = 15.0
var doom_timer = 0.0
var doom_active = false
var is_dead = false

func _ready():
	doom_duration = GameEvents.selected_doom_duration
...
func _air_dash():
	...
	# Dash with a slight upward bias
	...
	if dash_dir.y > 0:
		current_dash_speed = lerp(dash_force, jump_velocity, clamp(dash_dir.y, 0, 1))
...
func throw_mask():
	...
	if held_mask_visual:
		held_mask_visual.visible = false
		held_mask_visual.transform.origin = Vector3(0.3, -0.2, -0.5)
	charge_timer = 0.0
...
@onready var corpse_scene = preload("res://src/scenes/characters/corpse.tscn")
...
func die():
	...
	if was_controlled_local:
		# Player death physics: push head backward
		var head_offset = Vector3(0, 1.4, 0)
		var push_dir = global_transform.basis.z
		corpse.apply_impulse(push_dir * 30.0, head_offset)
	else:
		# NPC death physics
